...
export class AIAAssessmentServer {
  ...
  public analyzeProjectDescription(projectName: string, projectDescription: string): ProjectAnalysisResult {
  ...
  public assessProject(projectName: string, projectDescription: string, responses?: Array<{ questionId: string; selectedOption: number }>): AIAAssessment | { questions: any[] } {
    ...
    const aiaResponses: AIAResponse[] = responses.map(r => {
      ...
      if (!question) {
        throw new Error(`Unknown question ID: ${r.questionId}`);
      }
      if (r.selectedOption < 0 || r.selectedOption >= question.options.length) {
        throw new Error(`Invalid option for question ${r.questionId}`);
      }
      return {
  ...
  public getQuestions(category?: string, type?: 'risk' | 'mitigation'): AIAQuestion[] {
